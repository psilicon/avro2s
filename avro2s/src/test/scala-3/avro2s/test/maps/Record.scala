...
case class Record(var a: String) extends org.apache.avro.specific.SpecificRecordBase {
  ...
  private def encode$0(value$: _root_.avro2s.test.maps.Record, out$: _root_.org.apache.avro.io.Encoder): Unit = {
    encode$1(value$.`a`, out$)
  }

  private def decode$0(in$: _root_.org.apache.avro.io.ResolvingDecoder, reuse$: _root_.avro2s.test.maps.Record): _root_.avro2s.test.maps.Record = {
    val result$ = if (reuse$ == null) new _root_.avro2s.test.maps.Record() else reuse$
    decodeRecord$0(result$, in$)
    result$
  }

  private def decodeRecord$0(value$: _root_.avro2s.test.maps.Record, in$: _root_.org.apache.avro.io.ResolvingDecoder): Unit = {
...
object Record {
  val SCHEMA$: org.apache.avro.Schema = new _root_.org.apache.avro.Schema.Parser().parse("""{"type":"record","name":"Record","namespace":"avro2s.test.maps","fields":[{"name":"a","type":"string"}]}""")
}